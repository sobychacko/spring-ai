...
* **HTTP layer.** An `okhttp.requests` observation wraps every HTTP attempt to the Anthropic API, carrying HTTP method, URI, status code, and exception tags. Each request also propagates `traceparent` on the wire to any downstream services (AI gateways, OpenAI-compatible inference servers, proxies). OkHttp connection-pool gauges (`okhttp.pool.connection.count` with `state=active|idle` and `client.kind=sync|async`) are bound to the `MeterRegistry`.
...
For non-Boot applications, pass an `ObservationRegistry` and (optionally) a `MeterRegistry` to `AnthropicChatModel.builder()`:
...
AnthropicChatModel chatModel = AnthropicChatModel.builder()
    ...
    .meterRegistry(meterRegistry) // optional; binds connection-pool gauges
    .build();
----
