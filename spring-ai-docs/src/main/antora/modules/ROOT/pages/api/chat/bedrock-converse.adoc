...
You can also configure prompt caching using the framework-level `PromptCacheOptions`, which works across providers that support explicit cache control (e.g., Anthropic, AWS Bedrock):
...
The portable `PromptCacheStrategy` values map directly to `BedrockCacheStrategy`. When both `cacheOptions` (Bedrock-specific) and `promptCacheOptions` (portable) are set, the Bedrock-specific options take precedence.

NOTE: The `PromptCacheOptions.ttl` setting is ignored for Bedrock, which uses a fixed 5-minute cache TTL.
